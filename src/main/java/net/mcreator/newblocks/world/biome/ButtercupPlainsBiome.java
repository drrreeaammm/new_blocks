...
public class ButtercupPlainsBiome extends NewBlocksModElements.ModElement {
	public static Biome biome;

	public ButtercupPlainsBiome(NewBlocksModElements instance) {
		super(instance, 783);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(12638463).setWaterColor(-12813578).setWaterFogColor(-13603871)
						.withSkyColor(7972607).withFoliageColor(-7618234).withGrassColor(-7487412).build();
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.PLAINS).depth(0.1f).scale(0.1f)
						.temperature(1.2f).downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("new_blocks:buttercup_plains"));
			}
		}
	}
