...
public class BurntForestBiome extends NewBlocksModElements.ModElement {
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(1610608880).setWaterColor(-15764844).setWaterFogColor(-15960414)
						.withSkyColor(1610608880).withFoliageColor(-10466041).withGrassColor(-11902197)
						.setParticle(new ParticleEffectAmbience(ParticleTypes.ASH, 0.011f)).build();
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0.1f).scale(0f)
						.temperature(1.0999999999999999f).downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("new_blocks:burnt_forest"));
			}
		}
	}
