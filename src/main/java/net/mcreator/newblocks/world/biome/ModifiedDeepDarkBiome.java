...
public class ModifiedDeepDarkBiome extends NewBlocksModElements.ModElement {
	public static Biome biome;
	public ModifiedDeepDarkBiome(NewBlocksModElements instance) {
		super(instance, 718);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0.4f).scale(0.5f).temperature(0.5f)
						.downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("new_blocks:modified_deep_dark"));
			}
		}
	}
	@Override
