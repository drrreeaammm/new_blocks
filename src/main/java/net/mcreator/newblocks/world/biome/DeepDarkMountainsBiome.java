...
public class DeepDarkMountainsBiome extends NewBlocksModElements.ModElement {
	public static Biome biome;

	public DeepDarkMountainsBiome(NewBlocksModElements instance) {
		super(instance, 917);
			...
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-10066810).setWaterColor(-11646326).setWaterFogColor(-11906678)
						...
						.setMoodSound(new MoodSoundAmbience((net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS
								.getValue(new ResourceLocation("new_blocks:warden_distant")), 4000, 8, 2))
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0.6f).scale(0.7f).temperature(0.5f)
						.downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("new_blocks:deep_dark_mountains"));
			}
		}
	}
