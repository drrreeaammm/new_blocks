...
public class PumpkinPatchBiome extends NewBlocksModElements.ModElement {
	public static Biome biome;

	public PumpkinPatchBiome(NewBlocksModElements instance) {
		super(instance, 1027);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(12638463).setWaterColor(-16762044).setWaterFogColor(-16761020)
						.withSkyColor(7972607).withFoliageColor(-7892941).withGrassColor(-7304397).build();
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.PLAINS).depth(0f).scale(0f).temperature(0.4f)
						.downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("new_blocks:pumpkin_patch"));
			}
		}
	}
