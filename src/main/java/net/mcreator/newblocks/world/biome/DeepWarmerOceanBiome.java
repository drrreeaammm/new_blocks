...
public class DeepWarmerOceanBiome extends NewBlocksModElements.ModElement {
	public static Biome biome;

	public DeepWarmerOceanBiome(NewBlocksModElements instance) {
		super(instance, 644);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(12638463).setWaterColor(-10100749).setWaterFogColor(-8921360)
						.withSkyColor(7972607).withFoliageColor(-6237060).withGrassColor(-6237060).build();
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.OCEAN).depth(-1.6f).scale(0.1f)
						.temperature(0.5f).downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("new_blocks:deep_warmer_ocean"));
			}
		}
	}
