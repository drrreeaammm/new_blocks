...
import net.mcreator.newblocks.block.DeepslateBlock;
...
public class TheMoonDimension extends NewBlocksModElements.ModElement {
	public TheMoonDimension(NewBlocksModElements instance) {
		super(instance, 801);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(DeepslateBlock.block);
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("new_blocks:the_moon_biome")).getGenerationSettings()
	...
	public void clientLoad(FMLClientSetupEvent event) {
		DimensionRenderInfo customEffect = new DimensionRenderInfo(128, true, DimensionRenderInfo.FogType.NORMAL, false, false) {
			...
			public Vector3d func_230494_a_(Vector3d color, float sunHeight) {
				return new Vector3d(0.141176470588, 0.137254901961, 0.137254901961);
			}
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("new_blocks:the_moon"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
