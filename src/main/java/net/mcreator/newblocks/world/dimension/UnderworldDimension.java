...
public class UnderworldDimension extends NewBlocksModElements.ModElement {
	public UnderworldDimension(NewBlocksModElements instance) {
		super(instance, 1346);
	}
	...
	public void clientLoad(FMLClientSetupEvent event) {
		DimensionRenderInfo customEffect = new DimensionRenderInfo(Float.NaN, true, DimensionRenderInfo.FogType.NONE, false, false) {
			...
			public Vector3d func_230494_a_(Vector3d color, float sunHeight) {
				return new Vector3d(0.305882352941, 0.054901960784, 0.054901960784);
			}
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("new_blocks:underworld"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
