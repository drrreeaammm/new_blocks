...
public class BeneathEndCityShipStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
					...
					if ((random.nextInt(1000000) + 1) <= 11000) {
						...
						for (int a = 0; a < count; a++) {
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("new_blocks", "beneath_end_city_ship"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("beneath_end_city_ship"));
	...
	public static void addFeatureToBiomes(BiomeLoadingEvent event) {
		...
		if (new ResourceLocation("new_blocks:rock_bottom").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
