...
package net.mcreator.newblocks.world.structure;
...
import net.mcreator.newblocks.procedures.SoldierTent1AdditionalGenerationConditionProcedure;
...
public class AbandonedSoldierTent1Structure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 6000) {
						...
						for (int a = 0; a < count; a++) {
							...
							int y = spawnTo.getY();
							int z = spawnTo.getZ();
							if (!SoldierTent1AdditionalGenerationConditionProcedure.executeProcedure(Stream
									.of(new AbstractMap.SimpleEntry<>("world", world), new AbstractMap.SimpleEntry<>("x", x),
											new AbstractMap.SimpleEntry<>("y", y), new AbstractMap.SimpleEntry<>("z", z))
									.collect(HashMap::new, (_m, _e) -> _m.put(_e.getKey(), _e.getValue()), Map::putAll)))
								continue;
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("new_blocks", "solider_abandoned_tent"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("abandoned_soldier_tent_1"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("new_blocks:abandoned_soldier_tent_1"), configuredFeature);
		}
	}
	...
	public static void addFeatureToBiomes(BiomeLoadingEvent event) {
		...
		if (new ResourceLocation("new_blocks:burnt_forest").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("new_blocks:flower_plains").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("new_blocks:hills").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("new_blocks:flat_taiga").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("new_blocks:meadow").equals(event.getName()))
			biomeCriteria = true;
		if (new ResourceLocation("new_blocks:savanna_grassland").equals(event.getName()))
			biomeCriteria = true;
