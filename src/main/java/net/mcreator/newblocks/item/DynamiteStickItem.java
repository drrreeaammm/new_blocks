...
import net.mcreator.newblocks.procedures.DynamiteStickBulletHitsBlockProcedure;
...
public class DynamiteStickItem extends NewBlocksModElements.ModElement {
	@ObjectHolder("new_blocks:dynamite_stick")
	...
	public static class ItemRanged extends Item {
		public ItemRanged() {
			...
			setRegistryName("dynamite_stick");
		}
		...
		public void onPlayerStoppedUsing(ItemStack itemstack, World world, LivingEntity entityLiving, int timeLeft) {
			if (!world.isRemote && entityLiving instanceof ServerPlayerEntity) {
				...
				if (true) {
					ItemStack stack = ShootableItem.getHeldAmmo(entity, e -> e.getItem() == DynamiteStickItem.block);
					if (stack == ItemStack.EMPTY) {
						for (int i = 0; i < entity.inventory.mainInventory.size(); i++) {
							ItemStack teststack = entity.inventory.mainInventory.get(i);
							if (teststack != null && teststack.getItem() == DynamiteStickItem.block) {
								stack = teststack;
								break;
							}
						}
					...
					if (entity.abilities.isCreativeMode || stack != ItemStack.EMPTY) {
						ArrowCustomEntity entityarrow = shoot(world, entity, random, 0.6f, 0, 1);
						itemstack.damageItem(1, entity, e -> e.sendBreakAnimation(entity.getActiveHand()));
						if (entity.abilities.isCreativeMode) {
							entityarrow.pickupStatus = AbstractArrowEntity.PickupStatus.CREATIVE_ONLY;
						} else {
							if (new ItemStack(DynamiteStickItem.block).isDamageable()) {
	...
	public static class ArrowCustomEntity extends AbstractArrowEntity implements IRendersAsItem {
		...
		public ItemStack getItem() {
			return new ItemStack(DynamiteStickItem.block);
		}
