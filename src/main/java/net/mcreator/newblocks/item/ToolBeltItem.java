...
import net.mcreator.newblocks.gui.ToolBeltGUIGuiWindow;
import net.mcreator.newblocks.gui.ToolBeltGUIGui;
...
public class ToolBeltItem extends NewBlocksModElements.ModElement {
	...
	public void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == block) {
			if (Minecraft.getInstance().currentScreen instanceof ToolBeltGUIGuiWindow) {
			...
			if (entity instanceof ServerPlayerEntity) {
				NetworkHooks.openGui((ServerPlayerEntity) entity, new INamedContainerProvider() {
					...
					public ITextComponent getDisplayName() {
						return new StringTextComponent("Tool Belt");
					}
					...
					public Container createMenu(int id, PlayerInventory inventory, PlayerEntity player) {
						...
						return new ToolBeltGUIGui.GuiContainerMod(id, inventory, packetBuffer);
					}
				}, buf -> {
