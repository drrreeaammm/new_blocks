...
import net.mcreator.newblocks.entity.renderer.HazbandoRenderer;
...
public class HazbandoEntity extends NewBlocksModElements.ModElement {
	public static EntityType entity = (EntityType.Builder.<CustomEntity>create(CustomEntity::new, EntityClassification.MONSTER)
			.setShouldReceiveVelocityUpdates(true).setTrackingRange(64).setUpdateInterval(3).setCustomClientFactory(CustomEntity::new).immuneToFire()
			.size(0.6f, 1.8f)).build("hazbando").setRegistryName("hazbando");
	public HazbandoEntity(NewBlocksModElements instance) {
		super(instance, 169);
		FMLJavaModLoadingContext.get().getModEventBus().register(new HazbandoRenderer.ModelRegisterHandler());
	...
	public void initElements() {
		...
		elements.items.add(
				() -> new SpawnEggItem(entity, -16777216, -65536, new Item.Properties().group(ItemGroup.MISC)).setRegistryName("hazbando_spawn_egg"));
	}
	...
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		...
		if (!biomeCriteria)
			return;
		event.getSpawns().getSpawner(EntityClassification.MONSTER).add(new MobSpawnInfo.Spawners(entity, 11, 1, 1));
	}
	...
	private static class EntityAttributesRegisterHandler {
		...
		public void onEntityAttributeCreation(EntityAttributeCreationEvent event) {
			...
			ammma = ammma.createMutableAttribute(Attributes.MAX_HEALTH, 31);
