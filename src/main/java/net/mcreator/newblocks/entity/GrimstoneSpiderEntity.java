...
import net.mcreator.newblocks.procedures.WardenNaturalEntitySpawningConditionProcedure;
import net.mcreator.newblocks.entity.renderer.GrimstoneSpiderRenderer;
...
public class GrimstoneSpiderEntity extends NewBlocksModElements.ModElement {
	public static EntityType entity = (EntityType.Builder.<CustomEntity>create(CustomEntity::new, EntityClassification.MONSTER)
			.setShouldReceiveVelocityUpdates(true).setTrackingRange(64).setUpdateInterval(3).setCustomClientFactory(CustomEntity::new).immuneToFire()
			.size(1.4f, 0.9f)).build("grimstone_spider").setRegistryName("grimstone_spider");

	public GrimstoneSpiderEntity(NewBlocksModElements instance) {
		super(instance, 760);
		FMLJavaModLoadingContext.get().getModEventBus().register(new GrimstoneSpiderRenderer.ModelRegisterHandler());
	...
	public void initElements() {
		...
		elements.items.add(() -> new SpawnEggItem(entity, -14211289, -11908534, new Item.Properties().group(ItemGroup.MISC))
				.setRegistryName("grimstone_spider_spawn_egg"));
	}
	...
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		...
		if (!biomeCriteria)
			return;
		event.getSpawns().getSpawner(EntityClassification.MONSTER).add(new MobSpawnInfo.Spawners(entity, 10, 2, 3));
	}
	...
	private static class EntityAttributesRegisterHandler {
		...
		public void onEntityAttributeCreation(EntityAttributeCreationEvent event) {
			...
			ammma = ammma.createMutableAttribute(Attributes.ARMOR, 14);
