...
@OnlyIn(Dist.CLIENT)
public class VeryAngryWardenRenderer {
	public static class ModelRegisterHandler {
		@SubscribeEvent
		@OnlyIn(Dist.CLIENT)
		public void registerModels(ModelRegistryEvent event) {
			RenderingRegistry.registerEntityRenderingHandler(VeryAngryWardenEntity.entity, renderManager -> {
				return new MobRenderer(renderManager, new Modelwarden(), 1.4f) {
					@Override
					public ResourceLocation getEntityTexture(Entity entity) {
						return new ResourceLocation("new_blocks:textures/warden_very_angry.png");
					}
				};
			});
		}
	...
	public static class Modelwarden extends EntityModel<Entity> {
		...
		public void setRotationAngles(Entity e, float f, float f1, float f2, float f3, float f4) {
			...
			this.left_leg.rotateAngleX = MathHelper.cos(f * 1.1F) * -1.0F * f1;
			this.left_arm.rotateAngleX = MathHelper.cos(f * 0.6662F) * f1;
			this.right_leg.rotateAngleX = MathHelper.cos(f * 1.1F) * 1.0F * f1;
			this.body_r1.rotateAngleX = MathHelper.cos(f * 0.2F) * 0.2F * f1;
			this.body_r1.rotateAngleZ = MathHelper.cos(f * 0.2F) * 0.1F * f1;
			this.right_leg.rotateAngleZ = MathHelper.cos(f * 0.2F) * 0.2F * f1;
			this.left_leg.rotateAngleZ = MathHelper.cos(f * 0.2F) * 0.2F * f1;
			this.left_arm.rotateAngleX = MathHelper.cos(f * 0.2F) * -0.6F * f1;
			this.right_arm.rotateAngleX = MathHelper.cos(f * 0.2F) * -0.6F * f1;
			this.head.rotateAngleZ = MathHelper.cos(f * 0.3F) * 0.2F * f1;
			this.head.rotateAngleY = MathHelper.cos(f * 0.2F) * 0.2F * f1;
			this.head.rotateAngleX = MathHelper.cos(f * 0.3F) * 0.4F * f1;
			this.body_r1.rotateAngleX = MathHelper.cos(f * 0.1F) * 0.4F * f1;
			this.head.rotateAngleX = MathHelper.cos(f * 0.3F) * -0.2F * f1;
		}
	}
}
