...
import net.mcreator.newblocks.entity.AxolotlEntity;
...
public class AxolotlRenderer {
			...
			RenderingRegistry.registerEntityRenderingHandler(AxolotlEntity.entity, renderManager -> {
				return new MobRenderer(renderManager, new Modelaxolotl(), 0.5f) {
					...
					public ResourceLocation getEntityTexture(Entity entity) {
						return new ResourceLocation("new_blocks:textures/axolotl.png");
					}
				};
			});
		}
	...
	public static class Modelaxolotl extends EntityModel<Entity> {
		...
		public void setRotationAngles(Entity e, float f, float f1, float f2, float f3, float f4) {
			...
			this.top_gills.rotateAngleY = f3 / (180F / (float) Math.PI);
			this.top_gills.rotateAngleX = f4 / (180F / (float) Math.PI);
			this.front_right_leg.rotateAngleX = MathHelper.cos(f * 0.9F) * -0.2F * f1;
			this.left_gills.rotateAngleY = f3 / (180F / (float) Math.PI);
			this.left_gills.rotateAngleX = f4 / (180F / (float) Math.PI);
			this.tail.rotateAngleY = MathHelper.cos(f * 0.9F) * 0.7F * f1;
			this.back_right_leg.rotateAngleX = MathHelper.cos(f * 0.9F) * -0.2F * f1;
			this.right_gills.rotateAngleY = f3 / (180F / (float) Math.PI);
			this.right_gills.rotateAngleX = f4 / (180F / (float) Math.PI);
			this.back_left_leg.rotateAngleX = MathHelper.cos(f * 0.9F) * -0.2F * f1;
			this.front_left_leg.rotateAngleX = MathHelper.cos(f * 0.9F) * -0.2F * f1;
		}
	}
}
