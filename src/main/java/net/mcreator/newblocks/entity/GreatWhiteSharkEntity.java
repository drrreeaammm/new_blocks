...
import net.mcreator.newblocks.entity.renderer.GreatWhiteSharkRenderer;
import net.mcreator.newblocks.NewBlocksModElements;
import net.minecraft.entity.passive.DolphinEntity;

@NewBlocksModElements.ModElement.Tag
public class GreatWhiteSharkEntity extends NewBlocksModElements.ModElement {
	public static EntityType entity = (EntityType.Builder.<CustomEntity>create(CustomEntity::new, EntityClassification.WATER_CREATURE)
			.setShouldReceiveVelocityUpdates(true).setTrackingRange(64).setUpdateInterval(3).setCustomClientFactory(CustomEntity::new).size(3f, 2.1f))
					.build("great_white_shark").setRegistryName("great_white_shark");
	public GreatWhiteSharkEntity(NewBlocksModElements instance) {
		super(instance, 595);
		FMLJavaModLoadingContext.get().getModEventBus().register(new GreatWhiteSharkRenderer.ModelRegisterHandler());
	...
	public void initElements() {
		...
		elements.items.add(() -> new SpawnEggItem(entity, -12171706, -5395027, new Item.Properties().group(ItemGroup.MISC))
				.setRegistryName("great_white_shark_spawn_egg"));
	}

	@SubscribeEvent
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		event.getSpawns().getSpawner(EntityClassification.WATER_CREATURE).add(new MobSpawnInfo.Spawners(entity, 4, 1, 1));
	}
				...
				public void tick() {
					if (this.action == MovementController.Action.MOVE_TO && !CustomEntity.this.getNavigator().noPath()) {
						...
						if (CustomEntity.this.isInWater()) {
							...
							CustomEntity.this.setAIMoveSpeed(f1 * 0.06F);
						}
					} else {
