	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		...
		if (dependencies.get("x") == null) {
			if (!dependencies.containsKey("x"))
				NewBlocksMod.LOGGER.warn("Failed to load dependency x for procedure SculkTrapEntityWalksOnTheBlock!");
			return;
		}
		if (dependencies.get("y") == null) {
			if (!dependencies.containsKey("y"))
				NewBlocksMod.LOGGER.warn("Failed to load dependency y for procedure SculkTrapEntityWalksOnTheBlock!");
			return;
		}
			...
			new Object() {
				...
				private void run() {
					...
					if (entity instanceof LivingEntity) {
						((LivingEntity) entity).attackEntityFrom(new DamageSource("sculk.trap").setDamageBypassesArmor(), (float) 2);
					}
					...
					new Object() {
						...
						private void run() {
							{
								Entity _ent = entity;
								_ent.setPositionAndUpdate(x, (y + 0.4), z);
								if (_ent instanceof ServerPlayerEntity) {
									((ServerPlayerEntity) _ent).connection.setPlayerLocation(x, (y + 0.4), z, _ent.rotationYaw, _ent.rotationPitch,
											Collections.emptySet());
								}
							}
