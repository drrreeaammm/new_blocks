...
public class ShadowGrassBlock extends NewBlocksModElements.ModElement {
	...
	private static class FeatureRegisterHandler {
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
			...
			configuredFeature = feature.withConfiguration(
					(new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(block.getDefaultState()), new SimpleBlockPlacer())).tries(39)
							.build())
					.withPlacement(Placement.COUNT_NOISE.configure(new NoiseDependant(-0.8, 0, 19)));
			event.getRegistry().register(feature.setRegistryName("shadow_grass"));
	...
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		...
		if (new ResourceLocation("new_blocks:shadow").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
