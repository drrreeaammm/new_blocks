...
import net.mcreator.newblocks.procedures.DeeperDarkSculkVineAdditionalGenerationConditionProcedure;
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
			feature = new Feature<BlockClusterFeatureConfig>(BlockClusterFeatureConfig.field_236587_a_) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, BlockClusterFeatureConfig config) {
					...
					if (dimensionType == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("new_blocks:very_deep_dark")))
			...
			configuredFeature = feature.withConfiguration(
					(new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(block.getDefaultState()), new SimpleBlockPlacer())).tries(7)
							.build())
					.withPlacement(Features.Placements.PATCH_PLACEMENT).func_242731_b(33);
			event.getRegistry().register(feature.setRegistryName("deeper_dark_sculk_vine"));
	...
	public static class BlockCustomFlower extends SugarCaneBlock {
		public BlockCustomFlower() {
			super(Block.Properties.create(Material.PLANTS).tickRandomly().doesNotBlockMovement().sound(SoundType.PLANT)
					.hardnessAndResistance(0.6f, 0f).setLightLevel(s -> 0));
			setRegistryName("deeper_dark_sculk_vine");
		}
