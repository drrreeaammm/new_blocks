...
public class SculkVineBlock extends NewBlocksModElements.ModElement {
	@ObjectHolder("new_blocks:sculk_vine")
	public static final Block block = null;
	public SculkVineBlock(NewBlocksModElements instance) {
		super(instance, 160);
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, BlockClusterFeatureConfig config) {
					...
					for (int j = 0; j < 39; ++j) {
						...
						if (world.isAirBlock(blockpos)) {
							...
							int k = 1 + random.nextInt(random.nextInt(9) + 1);
							k = Math.min(9, k);
							for (int l = 0; l < k; ++l) {
			...
			configuredFeature = feature.withConfiguration(
					(new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(block.getDefaultState()), new SimpleBlockPlacer())).tries(17)
							.build())
					.withPlacement(Features.Placements.PATCH_PLACEMENT).func_242731_b(39);
			event.getRegistry().register(feature.setRegistryName("sculk_vine"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("new_blocks:sculk_vine"), configuredFeature);
		}
	}
	@SubscribeEvent
